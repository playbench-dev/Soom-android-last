...
public class NewCommunityMiddleWebFragment extends Fragment implements RadioGroup.OnCheckedChangeListener, AsyncResponse{

    private String TAG = "NewCommunityMiddleWebFragment";
    String mTag;
    String mTitle;
...
//    private FrameLayout frameLayout;
    ...
    public NewCommunityMiddleWebFragment() {

    }

    public static NewCommunityMiddleWebFragment newInstance(String mTag, String mTitle){
        NewCommunityMiddleWebFragment newMiddleFragment = new NewCommunityMiddleWebFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v                      = inflater.inflate(R.layout.fragment_new_middle_web_community, container, false);
        ...
        if (mTag.equals("1")){
            if (COMMUNITY_BLOG_BANNER_LIST == null){
                ...
                if (COMMUNITY_BLOG_BANNER_LIST.size() > 0){
                    ...
                    viewPagerBlogAdapter = new CommunityBannerAdapter(getActivity(), COMMUNITY_BLOG_BANNER_LIST);
                }
                TabCagegoryMake();
            }
            ...
            if (COMMUNITY_RUTIN_BANNER_LIST == null){
                ...
                if (COMMUNITY_RUTIN_BANNER_LIST.size() > 0){
                    ...
                    viewPagerRutinAdapter = new CommunityBannerAdapter(getActivity(), COMMUNITY_RUTIN_BANNER_LIST);
                }
                TabCagegoryMake();
            }
        }else{
    ...
    void TabCagegoryMake(){
            ...
            for (int i = 0; i < COMMUNITY_MUNU_NO_LIST.size(); i++){
                if (COMMUNITY_LV_LIST.get(i).equals("2")){
                    if (mTag.equals(COMMUNITY_PARENT_LIST.get(i))){
                        ...
                        if (mTag.equals("3")){
                            fragmentArrayList.add(new NewCommunityBlogFragment().newInstance(COMMUNITY_MUNU_NO_LIST.get(i), mTag,COMMUNITY_NAME_LIST.get(i)));
                        }else{
                            fragmentArrayList.add(new NewCommunityChildWebFragment().newInstance(COMMUNITY_MUNU_NO_LIST.get(i), mTag,COMMUNITY_NAME_LIST.get(i)));
                        }
        ...
        if (fragmentArrayList.size() > 0){
            ...
            if (mTag.equals("3")){
                fragmentArrayList.add(0,new NewCommunityBlogFragment().newInstance(mTag, mTag,"전체"));
            }else{
                fragmentArrayList.add(0,new NewCommunityChildWebFragment().newInstance(mTag, mTag,"전체"));
            }
            ...
            if (mTag.equals("3")){
                fragmentManager.beginTransaction().add(R.id.fragment_new_middle_community_layout, new NewCommunityBlogFragment().newInstance(mTag, mTitle,"")).commitAllowingStateLoss();
                fragmentManager.beginTransaction().show(new NewCommunityBlogFragment().newInstance(mTag, mTitle,"")).commitAllowingStateLoss();
            }else{
                fragmentManager.beginTransaction().add(R.id.fragment_new_middle_community_layout, new NewCommunityChildWebFragment().newInstance(mTag, mTitle,"")).commitAllowingStateLoss();
                fragmentManager.beginTransaction().show(new NewCommunityChildWebFragment().newInstance(mTag, mTitle,"")).commitAllowingStateLoss();
            }
        }
    }
...
//                radioButtonArrayList.get(0).setChecked(true);
            ...
            if (resultCode == RESULT_OK){
                Log.i(TAG,"data11 : " + data.getStringExtra("text"));
