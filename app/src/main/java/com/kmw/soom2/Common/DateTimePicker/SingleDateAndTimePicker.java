...
public class SingleDateAndTimePicker extends LinearLayout {
    ...
    public void setDisplayMinutes(boolean displayMinutes, String date) {
...
//        }, DELAY_BEFORE_CHECK_PAST);
    ...
    private void onClickTest(){
        final Date date = getDate();
        SimpleDateFormat ttDateFormat = new SimpleDateFormat("yyyy년 MM월 dd일");

        final CharSequence format = isAmPm ? FORMAT_12_HOUR : FORMAT_24_HOUR;
        final String displayed = DateFormat.format(format, date).toString();
        Log.i("calendar","display : " + displayed);
        Log.i("calendar","checkDate : " + checkDate);
...
//            try {
//                if (currentDate.after(ttDateFormat.parse(checkDate + " " + Utils.formatHHMM.format(date)))){
//                    for (OnCustomClick listener : customListeners) {
//                        listener.onDateChanged(displayed, date);
//                    }
//                }
...
//            }
    ...
    public void setMustBeOnFuture(boolean mustBeOnFuture) {
        ...
        if (!mustBeOnFuture) {
            maxDate = Calendar.getInstance().getTime(); //maxDate is Today
        }
    }
